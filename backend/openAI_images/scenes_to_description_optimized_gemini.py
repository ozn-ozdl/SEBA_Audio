...
def generate_video_description_with_gemini(video_file_path):
    ...
    # Create the prompt.
    prompt = '''
    You are an assistant that creates short, clear, and concise audio descriptions.
    Describe the video scenes, objects, actions, and transitions briefly in English without timestamps or introductory phrases. Directly provide the description of the content.   
    The description have to be one short, concise sentence that can be used as an audio description for visually impaired individuals.
    Please consider the whole video from the beginning to the end.
    '''
...
def describe_scenes_with_gemini_video(video_file, timestamps, output_dir):
    ...
    for idx, scene in enumerate(timestamps):
        ...
        command = (
            ...
            f" -ss {scene[0].get_timecode()} -to {scene[1].get_timecode()}"
