...
def split_video_by_scene_changes(scene_changes, frames_dir, frame_rate, video_file, output_dir):
    ...
    for idx, (start_time, end_time) in enumerate(timestamps):
        ...
        command = f"ffmpeg -i {video_file} -ss {start_time} -to {end_time} -c:v libx264 -c:a aac -strict experimental {output_video_file}"
        os.system(command)
...
def describe_scenes_with_gemini_video(video_file, scene_changes, frames_dir, frame_rate, output_dir):
    ...
    split_video_files,timestamps = split_video_by_scene_changes(scene_changes, frames_dir, frame_rate, video_file, output_dir)
...
Do not include introductory phrases like "Okay, here's the transcript and video description."
    '''
    model = genai.GenerativeModel(model_name="gemini-1.5-flash")
    response = model.generate_content([video_file, transcription_prompt], request_options={"timeout": 600})
    return response.text